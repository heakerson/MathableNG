...
describe('sin', () => {
    ...
    describe('Individual Methods', () => {
        ...
        describe('FlipSign', () => {
            ...
            const tests: FactorReplaceAndFlipSignTest[] = [
                new FactorReplaceAndFlipSignTest({ input: 'a^x', toStringBefore: 'sin[a^x]', toStringAfter: '-sin[a^x]' }),
                new FactorReplaceAndFlipSignTest({ input: 'a^b', toStringBefore: '-sin[a^b]', toStringAfter: 'sin[a^b]', sign: Sign.Negative }),
                new FactorReplaceAndFlipSignTest({ input: 'g^(a*(sin[a^(s-r*(p+(x/d)))])*b*x)', toStringBefore: 'sin[g^(a*(sin[a^(s-r*(p+(x/d)))])*b*x)]', toStringAfter: '-sin[g^(a*(sin[a^(s-r*(p+(x/d)))])*b*x)]' }),
            ];
