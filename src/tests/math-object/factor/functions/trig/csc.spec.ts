...
describe('Csc', () => {
    ...
    describe('Individual Methods', () => {
        ...
        describe('FlipSign', () => {
            ...
            const tests: FactorReplaceAndFlipSignTest[] = [
                new FactorReplaceAndFlipSignTest({ input: 'a^x', toStringBefore: 'csc[a^x]', toStringAfter: '-csc[a^x]' }),
                new FactorReplaceAndFlipSignTest({ input: 'a^b', toStringBefore: '-csc[a^b]', toStringAfter: 'csc[a^b]', sign: Sign.Negative }),
                new FactorReplaceAndFlipSignTest({ input: 'g^(a*(sin[a^(s-r*(p+(x/d)))])*b*x)', toStringBefore: 'csc[g^(a*(sin[a^(s-r*(p+(x/d)))])*b*x)]', toStringAfter: '-csc[g^(a*(sin[a^(s-r*(p+(x/d)))])*b*x)]' }),
            ];
