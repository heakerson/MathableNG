...
export class LogReplaceFlipSignTest extends FactorReplaceAndFlipSignTest {
    base?: string = undefined;

    constructor(props: Partial<LogReplaceFlipSignTest>) {
        super(props);
        Object.assign(this, props);
    }
}
...
describe('Log', () => {
    ...
    describe('Replace', () => {
        const standardBuilder = (test: LogReplaceFlipSignTest) => new Log(test.input, test.sign, test.base);
        const staticBuilder = (test: LogReplaceFlipSignTest) => {
            const contents = Factory.buildFactor(test.input);
            const base: any = test.base ? Factory.buildFactor(test.base) : undefined;
            return Log.fromFactors(contents, test.sign, base);
        };
        ...
        const tests: LogReplaceFlipSignTest[] = [
            new LogReplaceFlipSignTest({ input: 'a^x', toStringBefore: 'log[a^x,10]', toStringAfter: 'log[a^-z,10]'}),
            new LogReplaceFlipSignTest({ input: 'x^a', toStringBefore: 'log[x^a,y]', toStringAfter: 'log[-z^a,y]', base: 'y' }),
            new LogReplaceFlipSignTest({ input: 'a^b', toStringBefore: '-log[a^b,10]', toStringAfter: '-log[a^b,10]', sign: Sign.Negative }),
            new LogReplaceFlipSignTest({ input: 'g^(a*(sin[a^(s-r*(p+(x/d)))])*b*x)', toStringBefore: 'log[g^(a*(sin[a^(s-r*(p+(x/d)))])*b*x),10]', toStringAfter: 'log[g^(a*(sin[a^(s-r*(p+(-z/d)))])*b*x),10]' }),
        ];
        ...
        const rootTests: LogReplaceFlipSignTest[] = [
            new LogReplaceFlipSignTest({ input: 'y', toStringBefore: 'log[y,10]', toStringAfter: 'x' }),
            new LogReplaceFlipSignTest({ input: 'log[y]', toStringBefore: 'log[log[y,10],8]', toStringAfter: 'x', base: '8' }),
        ];
    ...
    describe('FlipSign', () => {
        ...
        const tests: LogReplaceFlipSignTest[] = [
            new LogReplaceFlipSignTest({ input: 'a^x', toStringBefore: 'log[a^x,10]', toStringAfter: '-log[a^x,10]'}),
            new LogReplaceFlipSignTest({ input: 'x^a', toStringBefore: 'log[x^a,y]', toStringAfter: '-log[x^a,y]', base: 'y' }),
            new LogReplaceFlipSignTest({ input: 'a^b', toStringBefore: '-log[a^b,10]', toStringAfter: 'log[a^b,10]', sign: Sign.Negative }),
            new LogReplaceFlipSignTest({ input: 'g^(a*(sin[a^(s-r*(p+(x/d)))])*b*x)', toStringBefore: 'log[g^(a*(sin[a^(s-r*(p+(x/d)))])*b*x),10]', toStringAfter: '-log[g^(a*(sin[a^(s-r*(p+(x/d)))])*b*x),10]' }),
            new LogReplaceFlipSignTest({ input: 'log[y]', toStringBefore: 'log[log[y,10],8]', toStringAfter: '-log[log[y,10],8]', base: '8' }),
        ];
